...
func (p *ProxyPostCommentHandler) SendError(msg string) {
	p.JSON(http.StatusOK, PostCommentResponse{
		BasicResponse: model.BasicResponse{StatusCode: 1, StatusMsg: msg}, Response: &service.Response{}})
}

func (p *ProxyPostCommentHandler) SendOk(comment *service.Response) {
	p.JSON(http.StatusOK, PostCommentResponse{
		BasicResponse: model.BasicResponse{StatusCode: 0, StatusMsg: "success"},
		Response:      comment,
	})
}
